  ...
  return (
    <div className={`flex flex-col justify-between bg-gray-500/10 pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
      <div>
        ...
        <button
                    className={`mt-8 flex items-center cursor-pointer transition-all duration-300 
                    ${expand ? "bg-[#e0edff] hover:bg-[#d0fff785] text-[rgb(209,255,26)] rounded-2xl gap-2 p-2.5 w-max" 
                            : "group relative h-9 w-9 mx-auto hover:bg-[#E0EDFF] rounded-lg"}`}
                >
                    <Image
                    ...
                    {/* Tooltip */}
                    <div className={`absolute w-max top-12 ${expand ? 'hidden' : 'right-12'} opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none`}>
                        New chat
                        <div className='w-3 h-3 absolute bg-black rotate-45 left-4 -bottom-1.5'></div>
                    </div>

                    {expand && <p className='text-[#000] font-medium'>New chat</p>}
                </button>
