    ...
    <div className={`flex flex-col justify-between bg-gray-500/10 pt-7 transition-all z-50 max-md:absolute max-md:h-screen ${expand ? 'p-4 w-64' : 'md:w-20 w-0 max-md:overflow-hidden'}`}>
      <div>
        <div className={`flex ${expand ? 'flex-row gap-10' : 'flex-col items-center gap-8'}`}>
          <Image
            ...
            src={expand ? assets.nla_text_logo_2 : assets.nla_text_logo_2}
            alt=''
          />
          <div
       ...
       <button className={`mt-8 flex items-center justify-center cursor-pointer ${
        expand
          ? "bg-gray/25 text-black/60 hover:opacity-90  rounded-2xl gap-2 p-2.5 w-max"
        ...
        <Image className={expand ? "w-6" : "w-7"}
          ...
          alt=""
        />
        <div className='absolute w-max -top-12 -right-12 opacity-0 hover:bg-black/10 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg  pointer-events-none'>
              New Chat 
      ...
      <div>
        <div
          ...
          >
            <div className='relative w-max bg-gray/23 text-black text-sm p-3 rounded-lg shadow-lg'>
                <Image src={assets.nla_qr_code} alt="" className='w-44' />
                <p>Scan to get Neurovision App</p>
                <div className={`w-3 h-3 absolute bg-gray/25 text-black rotate-45 left-4 -bottom-1.5 ${expand ? "right-1/2":"left-4"}`}></div>
            </div>
          </div>
          {expand && <><span className='text-black'>Get App </span> <Image alt="" src={assets.new_icon} /></>}
        </div>
        <div  onClick={user? null : openSignIn}
          
          className={`flex items-center ${expand ? 'hover:bg-black/10 rounded-lg border-solid border-b-black' : 'justify-center w-full'} gap-3 text-black text-sm p-2 mt-2 cursor-pointer`}>
          {user ? <UserButton />
